...
typedef piecewise_curve<double, double, true, memmo::eVector2> piecewise2_t;
typedef polynomial<double, double, true, memmo::eVector2> polynomial2_t;
typedef constant_curve<double, double, true, memmo::eVector2> constant2_t;
...
struct Spatial
{
  ...
  static inline eMatrixRot matrixRollPitchYaw(const double& roll, const double& pitch,
                                              const double& yaw)
  {
    double ca(cos(yaw));
    double sa(sin(yaw));
...
public:
   ...
   * @param swing_leg_height desired height at the apex of the trajectory wrt to the average
   * between init and end height
   * @param landing_advance duration before the final time where the feet pose should be reached
   */
...
private:
  ...
  ndcurves::polynomial1_t build_height_predef_trajectory(const double p, const double t_init,
                                                         const double t_end,
                                                         const double offset, const bool up);

  ndcurves::bezier1_t build_height_predef_lift(const double p, const double t_init,
                                               const double t_end);

  ndcurves::bezier1_t build_height_predef_landing(const double p, const double t_init,
                                                  const double t_end);

  void build_height_trajectory(const double& t_init, const double& t_end, const double& z_init,
                               const double& z_end, const double& v_init = 0.,
                               const double& a_init = 0., const double& j_init = 0.);

  void build_height_trajectory_minjerk(const double& t_init, const double& t_end,
                                       const double& z_init, const double& z_end,
                                       const double& v_init = 0., const double& a_init = 0.,
  ...
  void build_yaw_trajectory(const double& t_init, const double& t_end, const double& yaw_init,
                            const double& yaw_end, const double& delta_yaw_init = 0.,
                            const double& v_init = 0., const double& a_init = 0.);

  ndcurves::bezier1_t build_height_middle_trajectory(const double& t_init, const double& t_end,
                                                     const double p0, const double v0,
                                                     const double a0, const double j0,
