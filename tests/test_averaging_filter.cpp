...
TEST_F(AveragingFilterTest, checkConstructor) { AveragingFilter<double> obj; }

TEST_F(AveragingFilterTest, checkDefaultBehaviorWhenNoDataSent) {
  AveragingFilter<double> obj;
  for (unsigned int i = 0; i < 5; ++i) {
    ASSERT_EQ(obj.get_filtered_data(), 0.0);
  }
}
...
TEST_F(AveragingFilterTest, checkSimpleValues) {
  ...
  ASSERT_EQ(obj.get_filtered_data(), 3.0);
  ASSERT_EQ(obj.get_filtered_data(), average);
}
...
TEST_F(AveragingFilterTest, checkMaxSizeGetterAndSetter) {
  ...
  AveragingFilter<double> obj;
  obj.set_max_size(max_size);
  ASSERT_EQ(obj.get_max_size(), max_size);
}
...
TEST_F(AveragingFilterTest, checkFIFOsize) {
  ...
  for (unsigned int i = 1; i <= 2 * max_size; ++i) {
    obj.acquire(static_cast<double>(i));
    if (i <= max_size) {
      ASSERT_EQ(obj.get_buffer().size(), i);
    } else {
      ASSERT_EQ(obj.get_max_size(), obj.get_buffer().size());
    }
  }
  ASSERT_EQ(obj.get_max_size(), max_size);
}
...
TEST_F(AveragingFilterTest, checkFIFOcontent) {
  ...
  ASSERT_EQ(obj.get_filtered_data(), 8.0);
  ASSERT_EQ(obj.get_buffer()[0], 6.0);
  ASSERT_EQ(obj.get_buffer()[1], 7.0);
  ASSERT_EQ(obj.get_buffer()[2], 8.0);
  ASSERT_EQ(obj.get_buffer()[3], 9.0);
  ASSERT_EQ(obj.get_buffer()[4], 10.0);
}
...
TEST_F(AveragingFilterTest, checkEigenDefaultBehaviorWhenNoDataSent) {
  AveragingFilter<Eigen::VectorXd> obj;
  for (unsigned int i = 0; i < 5; ++i) {
    ASSERT_TRUE(obj.get_filtered_data().isApproxToConstant(0.0));
  }
}
...
TEST_F(AveragingFilterTest, checkEigenSimpleValues) {
  ...
  double average = (max_size + 1) * max_size * 0.5 / max_size;
  ASSERT_EQ(obj.get_filtered_data()(0), 3.0);
  ASSERT_EQ(obj.get_filtered_data()(0), average);
}
...
TEST_F(AveragingFilterTest, checkEigenFIFOcontent) {
  ...
  ASSERT_EQ(obj.get_filtered_data()(0), 8.0);
  ASSERT_EQ(obj.get_buffer()[0](0), 6.0);
  ASSERT_EQ(obj.get_buffer()[1](0), 7.0);
  ASSERT_EQ(obj.get_buffer()[2](0), 8.0);
  ASSERT_EQ(obj.get_buffer()[3](0), 9.0);
  ASSERT_EQ(obj.get_buffer()[4](0), 10.0);
}
