	...
	)
{
	...
	pinocchio::srdf::loadReferenceConfigurationsFromXML(
		pinocchio_model_complete_, iss_srdf, false);

	// Reduce the rigid body model and set initial position.
	if (!parseMovingJointNames(in_moving_joint_names_,
	                           in_controlled_joint_names_,
	                           moving_joint_names_,
	                           moving_joint_ids_,
	                           locked_joint_ids_)) {
		return false;
...
const std::vector<long unsigned int>& RobotModelBuilder::getMovingJointIds() const {
	return moving_joint_ids_;
}

const std::vector<long unsigned int>& RobotModelBuilder::getLockedJointIds() const {
	return locked_joint_ids_;
}

const std::string& RobotModelBuilder::getUrdf() const {
	return in_urdf_;
}
